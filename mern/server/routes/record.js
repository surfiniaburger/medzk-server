...
// Add a new route for updating user scores
router.post("/update-score", authenticateUser, async (req, res) => { //authenticateUser middleware added
  try {
    const { score } = req.body;
    const userId = req.user.id; // Access userId from decoded JWT


    if (typeof score !== 'number' || isNaN(score)) {
      return res.status(400).json({ error: "Invalid score value. Score must be a number." });
    }

    // Find the user and update their score. Use $inc to increment atomically
    const updateResult = await db.collection("users").updateOne(
      { _id: userId },
      { $inc: { score: score } } // Increment score, creates field if it doesn't exist
    );

      if (updateResult.modifiedCount === 0 && updateResult.upsertedCount === 0) { //check also upsertedCount if document was not present before.
      console.warn("User not found for score update:", userId); 
...
try {
  const leaderboard = await db.collection("users")
    .find({}, { projection: { _id: 0, name: 1, score: 1 } }) // Only retrieve name and score; exclude _id
...
// Authentication middleware to verify JWT (add this middleware to your protected routes)
const authenticateUser = (req, res, next) => {
  ...
  if (authHeader) {
    ...
    jwt.verify(token, process.env.JWT_SECRET, (err, user) => {
      ...
      req.user = user; // sets a property on the current request to refer to the user that sent the request (from JWT)
